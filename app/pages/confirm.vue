...
const redirectSuffix = route.fullPath.startsWith('/confirm')
  ? route.fullPath.slice('/confirm'.length)
  : ''

const destinationPath = `/auth/confirm${redirectSuffix}`

if (import.meta.server) {
  await navigateTo(destinationPath, { redirectCode: 301 })
} else {
    ...
    <main class="mx-auto flex min-h-screen w-full max-w-3xl items-center justify-center px-4 py-12 sm:px-6">
      <section class="w-full rounded-[32px] border border-white/10 bg-white/5 p-8 text-center shadow-[0_30px_80px_rgba(0,0,0,0.55)] sm:p-10">
        ...
        <p class="mt-5 text-sm text-white/80 sm:text-base">
          Veuillez patienter pendant la redirection vers la route de confirmation.
        </p>
      </section>
    </main>
  </div>
