...
// Produits fictifs codés en dur
const demoProducts = ref([
  { id: 1, name: "Pomme", size: "500g" },
  { id: 2, name: "Banane", size: "1kg" },
  { id: 3, name: "Lait", size: "1L" },
]);

onMounted(async () => {
  await props.store.getAllTasksIsTest();
});

const selectProduct = (target_product, best_match) => {
  props.store.setTargetProductAndBestMatch(target_product, best_match);
  router.push("/cheapest");
};

const scrollToCustomSearch = () => {
  router.push("/listing");
};
</script>
