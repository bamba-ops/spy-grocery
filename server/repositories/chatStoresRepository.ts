...
import { fetchProductStoreRows } from './storesRepository'

interface ChatStoreFacet {
  id: string
...
export const listChatStores = async (supabase: any): Promise<ChatStoreFacet[]> => {
  const rows = await fetchProductStoreRows(supabase)
  const map = new Map<string, ChatStoreFacet>()
